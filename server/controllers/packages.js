            ...
            Package.find({ _auctions: req.params.auctions })
              ...
              .exec(function(err, packages) {
                if (err) {
                  ...
                  Auction.findById(req.params.auctions, function(
                    err,
                    auctionDetails
                  ) {
                    if (err) {
          ...
          if (err) {
            ...
            Category.find({}, function(err, categories) {
              if (err) {
                ...
                Item.find({ _auctions: req.params.auctions }, function(
                  err,
                  items
                ) {
                  if (err) {
    ...
    if (globals.adminValidation(req, res)) {
      Package.findById(req.params.id, function(err, package) {
        ...
        if (err) {
          ...
          package.description =
            req.body.packageDescription || package.description;
          package.restrictions =
            req.body.packageRestrictions || package.restrictions;
      ...
      Package.findOne({ _id: req.params.id }, function(err, package) {
        if (err) {
            ...
            if (err) {
              ...
              Auction.findById(package._auctions, function(err, auction) {
                if (err) {
                  ...
                  auction._packages.splice(
                    auction._packages.indexOf(package._auctions),
                    1
                  );
                }
              });
          ...
          for (var i = 0; i < package._items.length; i++) {
            Item.update(
              { _id: package._items[i] },
              { $set: { packaged: false, _package: null } },
              function(err, result) {
                if (err) {
                  console.log(err);
                }
  ...
  this.priority = function(req, res) {
    if (req.params.priority < 1 || req.params.priority > 10) {
      res.json(
        `Priority ${
          req.params.priority
        } is invalid.  Please choose a unique priority value between 1 and 10.`
      );
    } else {
      //May be possible to not search priority for non-featured package
      Package.findOne(
        { _auctions: req.params.auctions, priority: req.params.priority },
        function(err, copy) {
          if (copy != null && req.params.featured === "true") {
            res.json(
              `Priority ${
                req.params.priority
              } is already in use.  Please choose a unique priority value.`
            );
          } else {
            Package.findById(req.params.id, function(err, result) {
              ...
              result.save(function(err) {
                if (err) {
                  console.log(err);
                  res.json(
                    `Error occurred while attempting to modify package id ${
                      result._id
                    }. Please try again.`
                  );
                } else {
